...
- El botón "Ingresar con Google" en la página de login es un mock — no usa OAuth real. Al hacer clic, autentica automáticamente con las credenciales de docente hardcodeadas en `authStore.ts`.
- El código del proyecto vive en branches de feature (no en `main`). Al crear un branch de trabajo, asegurar que se parte del branch con el código más reciente.
